...
//n-simo paso: utiliza ptcl para calcular e imprimir valores
static void energy(int myRank){
	...
	if(myRank == 0){
		...
		printf("time         Timesteps  n_act          E_pot        E_kin        E_tot        eerr            CPU_time \n");
...
//		printf("eta     = %.6E \n", eta);
	...
	// Paso 1: se envían los valores iniciales de las variables a todos los procesos
	...
	int n_loc = nbody/n_proc; //n_loc = n/p
	...
	for(int l=0; l<Particle::init_iter; l++){ //itera solo una vez
		// Paso 3: calcula objetos predictores jpred e ipred para cada partícula, necesarios para calcular force
#pragma omp parallel for
		for(int j=0; j<nj; j++){ //itera n/p veces
			jpred[j] = Predictor(time_cur, Jparticle(ptcl[j+jstart])); //cada proceso toma una porción distinta de ptcl
		}
#pragma omp parallel for
		for(int i=0; i<ni; i++){ //itera n veces
		...
		for(int i=0; i<ni; i++){ //todos los procesos toman la totalidad de ptcl
			// Paso 6: actualiza array de Partículas ptcl con los valores de force
...
*/
			...
			jptcl[i] = Jparticle(ptcl[i]); //jptcl es una copia de ptcl
		}
	...
	if(myRank == 0){
		...
		fprintf(stdout, "Initialized, %f Gflops\n", Gflops); //imprime el tiempo que tardó en llegar a este punto en Gflops
	}
	...
	// impresión del 2do resultado en adelante
	while(time_cur <= t_end){
		...
		int n_act = 0;
		while(t_plus_dt[n_act].first == min_t){ //cuenta cuántas partículas tienen su tiempo de próxima actualización igual a min_t
			active_list[n_act] = t_plus_dt[n_act].second; //o sea, las partículas que se deben a actualizar en este paso
			n_act++;
		}
#endif
...
#pragma omp parallel for
		...
		for(int j=0; j<nj; j++){ //itera n/p veces
			// ptcl[j+jstart+1].prefetch();
			jptcl[j+jstart+1].prefetch(); //no hace nada
...
#else
		   	jpred[j] = Predictor(min_t, jptcl[j+jstart]); //actualiza los predictores de las partículas correspondientes a este tiempo
#endif
		}
#endif
		int ni = n_act; //ni es el número de partículas cuyo tiempo de próxima actualización es la mínima posible, es decir, 
		//son las partículas que se van a actualizar en este paso

#pragma omp parallel for
		for(int i=0; i<ni; i++){ //itera el # de partículas que se van a actualizar en este paso
...
#else
			ipred[i] = Predictor(min_t, jptcl[active_list[i]]); //actualiza los predictores de las partículas correspondientes a este tiempo
#endif
#endif
		}
...
#pragma omp parallel for
		for(int i=0; i<ni; i++){ //itera el # de partículas que se van a actualizar en este paso
			ptcl[active_list[i+1]].prefetch(); //no hace nada
			// jptcl[active_list[i+1]].prefetch();
			Particle &p = ptcl[active_list[i]];

			p.correct(dt_min, dt_max, eta, force[i]); //actualiza las partículas correspondientes a este tiempo
/*
...
*/
...
#else
			jptcl[active_list[i]] = Jparticle(p); //actualiza jptcl con los valores de ptcl
#endif
		}
		...
		if(time_cur >= t_contr){
			//Paso 8: llama a energy, que utiliza los valores de ptcl calculados en este tiempo para imprimir
			// los valores de energía (time, timesteps, n_act, E_pot, E_kin, E_tot, eerr, CPU_time)
			energy(myRank);
			if(myRank == 0){
//			outputsnap_BH();
			}
